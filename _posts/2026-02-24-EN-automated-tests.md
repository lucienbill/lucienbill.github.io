...
It is all based on my personal and imperfect experience.
In other words: let's keep in mind that me declaring something with confidence doesn't necessarily means I'm right.

## Let's set the scene

How about me start with a story?

The Bananatronic Company (it obviously sells virtual connected bananas) wants to automated their tests. 
This is what they do:

They quickly assemble a team of 10 people and get them a piece of automation software that works well.

The infrastructure is not ready, the environment in the Bananatronic is quite complex to manage.
But the 10 people have already been hired, they are ready, so they might as well be busy: they set-up a parallel infrastructure, some sorte of simplified clone of the target infrastructure.

The fantastic 10 have one objective: take as many test cases from the existing test base and automated as many as they can.
...
- Then a very long feedback loop was use: nothing was tested against the target environment until there were many scripts to deliver
...
When I am why 9 strangers, who leads the show ?
...
What I really like about starting small is how easy it is to operate on consensus rather than hierarchy - to use a horizontal organisation rather than a vertical one.
In the field of software quality, it is an effective way to prevent great ideas from behind squashed by the weight of authority.
...
In short: take a few test cases and find a way to automated them that makes sense in your context.
Figure how to organise your code in a way that fits your needs.
...
When ask to perform a series of actions against an app, you can check whether the flow makes sense, whether the §user interface (UI) complies with the graphic mock-up or a predefined layout, and you can judge the quality of the user experience (UX).
...
And: the robot will no check what it hasn't been asked to check.
...
A set of test cases must be the answer to question "how can I verify whether the tested app complies with this requirement (business rule)?"

Note: because I tend to change things when I turn a manual test case into a script, I validate this change with coworkers.
If no one can "test" my test, then it is a problem

### Simple test cases

I often see complex scripts with convoluted logic branches, and sometimes polymorphism.

When I automated test I don't like complex frameworks, and the less I fiddle with object oriented programming the better.
